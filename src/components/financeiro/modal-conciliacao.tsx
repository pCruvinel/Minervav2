...
export function ModalConciliacao({ open, onClose, lancamento }: ModalConciliacaoProps) {
  ...
  // Auto-preencher setor quando categoria muda
  useEffect(() => {
  ...
  const handleCriar = async () => {
    if (!lancamento || !categoriaIdSelecionada || !setorSelecionado) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col p-0">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'vincular' | 'criar')} className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Tab: Vincular */}
          <TabsContent value="vincular" className="flex-1 overflow-hidden flex flex-col m-0 px-6 py-4">
            {/* Search */}
        ...
        <DialogFooter className="px-6 py-4 border-t bg-muted/20">
          ...
          {!isReadOnly && (
            <Button
              ...
              disabled={
                activeTab === 'vincular'
                  ? !registroSelecionado || vincularMutation.isPending
                  : !categoriaIdSelecionada || !setorSelecionado || !isRateioValido || classificarMutation.isPending
              }
              className="gap-2"
            >
              <Check className="h-4 w-4" />
              {vincularMutation.isPending || classificarMutation.isPending ? 'Conciliando...' : 'Conciliar'}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
