...
flowchart TD
    ...
    subgraph "☁️ Vercel Edge Network"
        B1[Vite + React Application]
        B2[Client Components - SPA]
    end

    subgraph "🗄️ Supabase Platform"
        C1[PostgreSQL Database<br/>• ordens_servico, os_etapas<br/>• colaboradores, clientes<br/>• turnos, agendamentos<br/>• Row Level Security]
        C2[Authentication<br/>• JWT + Refresh Tokens<br/>• OAuth providers<br/>• Password recovery]
        C3[Real-time Engine<br/>• WebSocket connections<br/>• Live subscriptions<br/>• Broadcast updates]
        C4[Storage<br/>• Documentos PDF<br/>• Imagens de perfil<br/>• Anexos de OS]
        C5[Edge Functions<br/>• Geração de PDF<br/>• Processamento de webhooks<br/>• APIs customizadas]
    end

    A1 -->|HTTPS/WebSocket| B1
    B1 --> B2

    B2 -->|Supabase Client| C1
    B2 -->|Supabase Client| C2
    B2 -->|Supabase Client| C3
    B2 -->|Supabase Client| C4
    B2 -->|Supabase Client| C5
