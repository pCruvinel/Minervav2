...
export type RoleLevel =
  | 'DIRETORIA'              // Nível 4
  | 'GESTOR_ADMINISTRATIVO'       // Nível 3
  | 'GESTOR_ASSESSORIA'      // Nível 3
  | 'GESTOR_OBRAS'           // Nível 3
  | 'COLABORADOR_ADMINISTRATIVO'  // Nível 2
...
- **Descrição**: Gerente do setor administrativo
...
- **Descrição**: Colaborador do setor administrativo
- **Setor**: COM
- **Características**:
...
- **Setor**: ASS
- **Características**: Idênticas ao Colaborador Administrativo
...
```
          ...
          │                    │                    │
┌─────────┴─────────┐  ┌──────┴────────┐  ┌───────┴────────┐
│ GESTOR_ADMINISTRATIVO  │  │    GESTOR     │  │     GESTOR     │
...
WITH CHECK (
  EXISTS (
    SELECT 1 FROM colaboradores
    WHERE id = auth.uid()
    AND role_nivel IN ('GESTOR_ADMINISTRATIVO', 'GESTOR_ASSESSORIA', 'GESTOR_OBRAS', 'DIRETORIA')
  )
);
...
-- CORRETO
WHERE colaboradores.role_nivel IN ('DIRETORIA', 'GESTOR_ADMINISTRATIVO')
```
...
#### 5. Lógica de Delegação do Gestor Administrativo
**Problema**: Gestor Administrativo pode delegar para todos os setores, mas seu setor é COM
...
SELECT
  ...
  CASE
    WHEN c.role_nivel IN ('DIRETORIA', 'GESTOR_ADMINISTRATIVO', 'GESTOR_ASSESSORIA', 'GESTOR_OBRAS') THEN true
    ELSE false
