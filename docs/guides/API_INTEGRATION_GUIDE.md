...
O backend implementa uma camada de normalização para garantir compatibilidade entre os tipos do Frontend e os ENUMs do Banco de Dados.

- **Status de Etapa**: Converte `Concluída` -> `APROVADA`, `Reprovada` -> `REJEITADA`.
- **Status Geral**: Converte `Aguardando Aprovação` -> `EM_VALIDACAO`.
- **Status Cliente**: Converte `Ativo` -> `CLIENTE_ATIVO`.

Para detalhes técnicos completos, consulte: [BACKEND_NORMALIZATION.md](../technical/BACKEND_NORMALIZATION.md) e [INCONSISTENCIES_REPORT.md](../technical/INCONSISTENCIES_REPORT.md).
