...
| C-1 | Feriados — entidade `feriados` no banco + marcar dias indisponíveis | ✅ | `celula-mes.tsx` filtra bloqueios por `motivo === 'feriado'` com badge vermelho 🏛️. `modal-detalhes-dia.tsx` exibe seção "Feriados e Bloqueios". `use-dias-uteis.ts` usa RPC `contar_dias_uteis_mes` que desconta feriados |
...
| C-4 | Cálculo de custo por dia útil — config dias úteis/mês | ✅ | `use-dias-uteis.ts` implementa hook que calcula dias úteis reais via RPC (desconta feriados e fins de semana) |
...
| R11-5 | 🆕 Remover campo "Tipo de Empresa" do cadastro de Lead | ⬜ | Val: "essa informação não é relevante". Campo `tipoEmpresa` ainda presente em `lead-form-identificacao.tsx`, `cadastrar-lead.tsx` L977-993, `lead-cadastro.tsx`, e `types.ts` |
| R11-6 | 🆕 Cadastro de Lead — adicionar sub-pergunta "Quantos elevadores?" quando "Possui elevador? = Sim" | ✅ | `lead-form-edificacao.tsx` L238-253: campo `qtdElevadores` exibido condicionalmente quando `data.possuiElevador === true`, com `handleNumericChange` e limpar ao desativar |
| R11-7 | 🆕 Cadastro de Lead — adicionar sub-pergunta "Quantas piscinas?" quando "Possui piscina? = Sim" | ✅ | `lead-form-edificacao.tsx` L276-291: campo `qtdPiscinas` exibido condicionalmente quando `data.possuiPiscina === true`, com `handleNumericChange` e limpar ao desativar |
| R11-8 | 🆕 Bug: edificação tipo "condomínio residencial de casas" → não deve perguntar "quantidade de pavimentos" | ✅ | `lead-form-edificacao.tsx` L81-83: `isCasas = tipoEdificacao === 'Condomínio Residencial - Casas'` → `showPavimentos = !isCasas`. Campo oculto corretamente |
| R11-9 | 🆕 Bug: campo numérico no cadastro de Lead não aceita valores de 2 a 9 (só aceita 1 ou ≥10) | ✅ | `lead-form-edificacao.tsx` L67-76: `handleNumericChange` usa `type="text"` + `inputMode="numeric"` + sanitização manual `replace(/\D/g, '')`. Comentário documenta fix R11-9 |
| R11-10 | 🆕 Bug: CNPJ sem formatação no cadastro de Lead | ✅ | `lead-form-identificacao.tsx` L22: importa `FormMaskedInput` com `validarCNPJ`. L108-118: campo CPF/CNPJ usa `FormMaskedInput` com máscara + validação automática |
| R11-11 | 🆕 Bug: cadastro de Lead congela a tela (não conclui/salva) | ⬜ | Val: "tentei cadastrar um lead e ficou travado, congelou". Afeta OS-01 a OS-04. Pedro confirmou bug — investigar `lead-cadastro.tsx` save flow |
...
| ID | Tarefa | Status | Contexto / Arquivos |
|----|--------|--------|---------------------|
| #7 | Fluxo de Caixa — Refatorar queries | ⬜ | `fluxo-caixa-page.tsx` existe; hook `use-fluxo-caixa.ts` com `useFluxoCaixaKPIs` e `useFluxoMensal` implementados. Integrado no `financeiro-dashboard-page.tsx`. Funcionalidade reportada como parcialmente operacional |
...
| #20 | Despesas — Filtrar apenas categoria "despesa" | ⬜ | `gestao-despesas-page.tsx` (32KB); DB: `categorias_financeiras` (15 rows). Filtro não verificado |
...
| R11-13 | 🆕 Painel financeiro — exibir indicadores: receber hoje, a pagar hoje, projeção 30 dias, saldo atual | ✅ | `financeiro-dashboard-page.tsx` (26KB) com cards KPI. `kpi-card-financeiro.tsx`, `kpi-card-comparativo.tsx`, `kpi-financeiro-grid.tsx` implementados |
...
| ID | Tarefa | Status | Contexto / Arquivos |
|----|--------|--------|---------------------|
| R11-18 | 🆕 Clique no item de custo do CC → exibir detalhamento de tudo conciliado naquela categoria/CC | ⬜ | `centro-custo-detalhes-page.tsx` (25KB) existe com drill-down parcial. Funcionalidade de detalhamento por categoria em construção |
| R11-19 | 🆕 Mão de obra no CC puxa valor da presença diária (não conciliação) | ⬜ | Val: "com exceção da mão de obra que vai puxar da presença diária". `custo-mao-de-obra-page.tsx` (21KB) existe mas integração CC ↔ presença não confirmada |
...
| ID | Tarefa | Status | Contexto / Arquivos |
|----|--------|--------|---------------------|
| #9 | Alterar fluxo de cadastro de colaborador | ✅ | `modal-cadastro-colaborador.tsx` (51KB) — Zod, ViaCEP, cálculos CLT/contrato |
| #10 | Documentos fixos e predefinidos | ✅ | `colaboradores_documentos`; payload tem `documentos_obrigatorios: string[]` |
| #11 | Solicitação mão de obra — tela congelada | ⬜ | `recrutamento/` (12 arquivos) com kanban e modals. Problema de re-render reportado |
| #12 | Presença — CC sem divisão não lista opções 1-7 | ⬜ | `controle-presenca-page.tsx` (39KB); `alocacao_horas_cc` (40 rows). Filtro CC não verificado |
...
| #33 | Página do Colaborador (filtros, exportar) | ✅ | `colaborador-detalhes-page.tsx` (60KB) + `presenca-historico-colaborador-page.tsx` + tabs |
...
| #40 / RH-11 | Minutos → Horas (>60 min) | ✅ | `formatMinutosAtraso` em `lib/constants/colaboradores.ts` L188-194: converte <60→"X min", =60→"1h", >60→"Xh Ymin". Usado em 4 componentes de presença |
| #41 / RH-12 | Botão "Voltar" no detalhamento de frequência | ⬜ | `presenca-detalhes-page.tsx` (36KB) — nenhum botão "Voltar" encontrado no código |
...
| #45 / G-1 | Status "Cancelado" para OS | ✅ | `status-machine.ts` L23,66: estado `cancelado` como terminal em `OS_TRANSITIONS`. `types.ts` L42,114,328: `cancelado` na enum `StatusGeral` e `STATUS_LABELS`. Contratos page L216 inclui filtro "Cancelado" |
...
| R11-23 | 🆕 OS-10 (recrutamento): unificar formulário numa única página (remover separação em etapas) | ⬜ | `os-10/` tem 3 subpastas (`components/`, `pages/`, `steps/` com 5 steps). Val: "ficar tudo na mesma página". Requer refatoração de steps → single page |
...
| R11-25 | 🆕 OS-10 (recrutamento): adicionar subcategoria de qualificação por tipo de vaga | ⬜ | Campo `subcategoria` não encontrado no código OS-10 |
| R11-26 | 🆕 OS-10 (recrutamento): adicionar campo "expectativa de início" (dias) por vaga | ⬜ | Campo `expectativa_inicio` não encontrado no código |
...
| #17 | Diário de Obras — integração API + exibição na página do cliente | ⬜ | API já implementada no backend (Pedro confirmou), mas **front-end não existe**. Nenhum ficheiro `diario-de-obras` ou `diarioDeObras` encontrado em `src/`. Relatórios aprovados devem aparecer na página do cliente |
...
| R11-36 | 🆕 Página de contratos — OS de cada contrato, faturas, centro de custos, histórico de OS | ⬜ | Pedro: "a página do contrato vai ser onde a partir desse contrato você vai abrir aquelas outras OS". `contratos/` tem 4 arquivos com listagem básica |
...
### Alterações nesta revisão (25/03/2026)

| ID | De | Para | Evidência |
|----|----|------|-----------|
| R11-6 | ⬜ | ✅ | `lead-form-edificacao.tsx` L238-253: sub-pergunta "Quantos elevadores?" implementada |
| R11-7 | ⬜ | ✅ | `lead-form-edificacao.tsx` L276-291: sub-pergunta "Quantas piscinas?" implementada |
| R11-8 | ⬜ | ✅ | `lead-form-edificacao.tsx` L81-83: pavimentos ocultos para "Condomínio Residencial - Casas" |
| R11-9 | ⬜ | ✅ | `lead-form-edificacao.tsx` L72-76: `handleNumericChange` com `type="text"` + `inputMode="numeric"` |
| R11-10 | ⬜ | ✅ | `lead-form-identificacao.tsx` L22,108-118: `FormMaskedInput` com `validarCNPJ` para máscara de CNPJ |
| #40 | ⬜ | ✅ | `formatMinutosAtraso` em `lib/constants/colaboradores.ts` converte >60min para "Xh Ymin" |
| #45 / G-1 | ⬜ | ✅ | `status-machine.ts`: `cancelado` como estado terminal em `OS_TRANSITIONS` |
| C-1 | ⬜ | ✅ | `celula-mes.tsx`, `modal-detalhes-dia.tsx`, `use-dias-uteis.ts`: feriados integrados no calendário |
| C-4 | ⬜ | ✅ | `use-dias-uteis.ts`: hook calcula dias úteis/mês via RPC `contar_dias_uteis_mes` |
...
> - #7 Fluxo de Caixa — hook e route existem, mas funcionalidade completa não confirmada
